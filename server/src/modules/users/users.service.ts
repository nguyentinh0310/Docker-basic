...
import { IPagination, TokenData } from '@core/interfaces';
...
class UserService {
  ...
  public async getAllPaging(keyword: any, page: number): Promise<IPagination<IUser>> {
    let query = {};
    ...
    const rowCount = await this.userSchema.countDocuments(query).exec();
